var admin = '76561198014938668'; // STEAM ID 64 OF YOUR MAIN ACCOUNT WHERE YOU WANT TO RECIEVE YOUR COMMISSION : GET IT AT HTTP://WWW.STEAMID.IO/

// CSGO.NETWORK - New information required

var botsteamid ='76561198324597284'; // STEAM ID 64 OF YOUR BOT ACCOUNT : GET IT AT HTTP://WWW.STEAMID.IO/
var shared_secret ='S3AsEQX9S70VjkjZJrJommJYdwo='; // SHARED SECRET CODE YOU CAN GET IT THROUGH : 2FA / STEAM DESKTOP AUTHENTICATOR ( BOTH OPTIONS ARE INCLUDED )
var identity_secret='eGbQXIg7cl1MrVfIjIoNt2N/ogU='; // IDENTITY SECRET CODE YOU CAN GET IT THROUGH : 2FA / STEAM DESKTOP AUTHENTICATOR ( BOTH OPTIONS ARE INCLUDED )
...
var logOnOptions = {
    accountName: 'losujkupowanie', // BOT ACCOUNT LOGIN USERNAME
    password: 'TylkoKupuje69!', // BOT ACCOUNT LOGIN PASSWORD
...
var device_id = 'android:' + hash; // CSGO.NETWORK - This is the only part I'm concerned about, just because it says ,,--10"
...
sitename = "losujskin.pl"; // PUT YOUR SITE NAME HERE , PUT IT LIKE THIS STRUCTURE : YOURSITE.COM
...
var apik = "62BF9BF92E32493C982E74845C59018B"; // GO ON THE BOTS ACCOUNT AND CREATE AN API KEY ( IT HAS TO BE THROUGH THE BOTS ACCOUNT )

var mysqlInfo;
mysqlInfo = {
    host     : 'localhost', // MYSQL , LEAVE IT AS LOCALHOST IF YOU RUN IT ON THE SAME SERVER AS THE WEBSITE AND DATABASE
    user     : 'root', // MYSQL USERNAME
    password : 'robhajsy69', // MYSQL PASSWORD
    database : 'losujskiny', // MYSQL DATABASENAME
    charset  : 'utf8_general_ci'
};

//var mysqlConnection = mysql.createConnection('mysql://losujskiglroot:Robhajsy100@phpmyadmin.ovh.net/losujskiglroot?debug=true&charset=BIG5_CHINESE_CI&timezone=-0700&port=3306');
        ...
        mysqlConnection.query('SELECT * FROM `queue` WHERE `status`=\'active\'', function(err, row, fields) {
            ...
            for(var i=0; i < row.length; i++) {
                ...
                if (num > 0) {
                    ...
                    offers.makeOffer ({
                        partnerAccountId: row[i].partnerid,
                        itemsFromMe: item,
                        accessToken: row[i].tokenurl,
                        itemsFromThem: [],
                        message: row[i].token
...
steam.on('loggedOn', function(result) {
    ...
    //steam.sendMessage(admin,"MARIAN!!");
});
                        ...
                        offers.loadPartnerInventory({partnerSteamId: offer.steamid_other, appId: 730, contextId: 2, tradeOfferId: offer.tradeofferid, language: "en"}, function(err, hitems) {
                            ...
                            setTimeout(function() {
                                ...
                                mysqlConnection.query('SELECT ban FROM `users` WHERE `steamid`=\''+offer.steamid_other+'\'', function(err, row, fields)
                                {
                                    ...
                                    // CSGO.NETWORK - This part grabs the trade url of the person and gets their token because it is needed to get their escrow duration.
                                    var tradelink;
        ...
        if (
            ...
            body.response.trade_offers_sent.forEach(function(offer) {
                ...
                switch (offer.trade_offer_state) {
                    ...
                    case 9:
                    case 10:
                        console.log('Oferta odrzucona');
                        //console.log(offer.message);
                        //console.log(offer.tradeofferid);
                        mysqlConnection.query('UPDATE `queue` SET `status`= "odrzucone" WHERE `token` = \'' + offer.message + '\'', function(err, row, fields) {});
                        mysqlConnection.query('SELECT * FROM `queue` where token = \'' + offer.message + '\'', function(err, rows) {
                            if (err) {
                                return;
                            }
                            if (rows.length > 0) {
                                id_wymiany = (rows[0].wylosowane);
                                mysqlConnection.query('UPDATE `wylosowane` SET `wydane`= 0 WHERE `id` = \'' + id_wymiany + '\'', function (err, row, fields) {
                                });
                            } else {
                                return;
                            }
                        ...
                        break;
                    case 3:
                        console.log('Oferta zaakceptowana');
                        //console.log(offer.message);
                        //console.log(offer.tradeofferid);
                        mysqlConnection.query('UPDATE `queue` SET `status`= "akceptowane" WHERE `token` = \'' + offer.message + '\'', function(err, row, fields, result) {
                            mysqlConnection.query('SELECT * FROM `queue` where token = \'' + offer.message + '\'', function(err, rows) {
                                if(err) {
                                    return;
                                }
                                if (rows.length > 0) {
                                    id_wymiany = (rows[0].wylosowane);
                                    mysqlConnection.query('UPDATE `wylosowane` SET `wydane`= "1" WHERE `id` = \'' + id_wymiany + '\'', function (err, row, fields) {
                                    });
                                }else {
                                    return;
                                }
        ...
        if (
            ...
            body.response.trade_offers_sent.forEach(function(offer) {
                ...
                switch (offer.trade_offer_state) {
                    ...
                    case 11:
                        console.log(offer.message);
                        mysqlConnection.query('UPDATE `queue` SET `status`= "wyslane" WHERE `token` = \'' + offer.message + '\'', function(err, row, fields) {
                            if(err) {
                                return;
                            } else {
                                console.log('Oferta wyslana');
                            }
                        });
                        break;
                    default:
        ...
        if (
            ...
            body.response.trade_offers_received.forEach(function(offer) {
                if (offer.trade_offer_state === 2) {
                        ...
                        console.log('Oferta admina zaakceptowana');
                }
            });
        }
    });
...
setInterval(function () {
    mysqlConnection.query('SELECT * from queue', function(err){console.log(err);});
    //zmianaStatusu();
    akceptowanieOfert();
    //sendoffers();
    make_offer_error();
