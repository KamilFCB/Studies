...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ...
  (let ([encrypted-message (RSA-encrypt message recipient-public-key)])
    (let ([compressed (compress encrypted-message)])
      (let ([signature (RSA-transform compressed sender-private-key)])
        (make-signed-message encrypted-message signature)))))
    

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  ;;; XXX: Write your code here
  (let ([msg (message cyphertext)]
        [sgn (signature cyphertext)])
        (let ([decrypted-signature (RSA-transform sgn sender-public-key)])
              (if (= decrypted-signature (compress msg))
                  (RSA-decrypt msg recipient-private-key)
                  (display "Błędny podpis")))))
