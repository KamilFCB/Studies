...
(define new-eval
  '(lambda-rec (eval e)
               (if (pair? e)
                   (let (first (eval (car (cdr e))))
                     (let (second (eval (car (cdr (cdr e)))))
                       (let (op (car e))
                       (cond [(eq? '- op) ((lambda (a b) (- a b)) first second)]
                             [(eq? '+ op) ((lambda (a b) (+ a b)) first second)]
                             [(eq? '/ op) ((lambda (a b) (/ a b)) first second)]
                             [(eq? '* op) ((lambda (a b) (* a b)) first second)]))))
                   e)))
...
; tests
(eval (list new-eval '3))
(eval (list new-eval '(cons '+ (cons 1 (cons 3 null)))))
(eval (list new-eval '(cons '+ (cons (cons '+ (cons 10 (cons 15 null))) (cons 3 null)))))
(eval (list new-eval '(cons '* (cons (cons '* (cons 10 (cons 15 null))) (cons 3 null)))))
(eval (list new-eval '(cons '- (cons (cons '- (cons 10 (cons 15 null))) (cons 3 null)))))
(eval (list new-eval '(cons '/ (cons (cons '/ (cons 10 (cons 15 null))) (cons 3 null)))))
(eval (list new-eval '(cons '/ (cons (cons '/ (cons 10 (cons 15 null))) (cons (cons '+ (cons 1 (cons 3 null))) null)))))
(eval (list new-eval '(cons '+ (cons (cons '- (cons 42 (cons 31 null))) (cons (cons '* (cons 11 (cons 4 null))) null)))))
