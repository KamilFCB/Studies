...
(define (make-lambda e)
  (lambda-cons '() e))

(define (substitute e env)
  (cond [(const? e) e]
        [(and (is-in-env? e env) (var? e)) (find-in-env e env)]
        [(let? e) (let ([new-let-def (substitute (let-def-expr (let-def e)) env)])
                    (let-cons (list (let-def-var (let-def e)) new-let-def)
                              (substitute (let-expr e) (add-to-env (let-def-var (let-def e)) new-let-def env))))]
        [(lazy-let? e) (let ([new-lazy-let-def (substitute (lazy-let-def-expr (lazy-let-def e)) env)])
                         (lazy-let-cons (list (lazy-let-def-var (lazy-let-def e))  new-lazy-let-def)
                                        (substitute (lazy-let-expr e) (add-to-env
                                                                       (lazy-let-def-var (lazy-let-def e))
                                                                       new-lazy-let-def
                                                                       env))))]                         
        [else (cons (car e)(map (lambda (x) (if (and (is-in-env? x env) (var? x))
                                                (find-in-env x env)
                                                (substitute x env))) (cdr e)))]))
  ...
  (cond [(const? e)
        ...
        [(op? e)
         (apply (op->proc (op-op e))
                (map (lambda (a) (if (lambda? a) (eval-env (list a) env) (eval-env a env)))
                     (op-args e)))]
        [(let? e)
         (eval-env (let-expr e)
        ...
        [(lazy-let? e)
         (eval-env (lazy-let-expr e)
                   (add-to-env (lazy-let-def-var (lazy-let-def e))
                               (make-lambda (substitute (lazy-let-def-expr (lazy-let-def e)) env))
                               env))]
        [(my-null? e)
         null]
        [(cons? e)
        ...
        [(var? e)
         (let ((v (find-in-env (var-var e) env)))
           (if (lambda? v)
               (eval-env (list v) env)
               v))]
        [(lambda? e)
...
(eval (list '(lambda-rec (fib a)
                         (lazy-let [a1 0]
                         (lazy-let [a2 1]
                         (lazy-let [a3 1]
                         (lazy-let [n (+ (fib (- a 1)) (fib (- a 2)))]
                         (cond [(= a 0) a1]
                               [(= a 1) a2]
                               [(= a 2) a3]
                               [(> a 2) n]))))))
...
(eval '(lazy-let [x (/ 4 0)]
       (lazy-let [y (lazy-let [x 3] x) ]
       (let [x 10] y))))

(eval '(lazy-let [x (/ 4 0)]
       (lazy-let [y (let [x 3] x)]
       (let [ x 10] y))))